...
import { FileTextOutlined, LikeOutlined, EyeOutlined, CommentOutlined } from "@ant-design/icons";

import { postNewsForReporter, getCategoryAPI } from "~/api";
...
function NewsMangeReporter() {
  ...
  const currentRecommends = news.slice(startIndex, startIndex + pageSize);

  const user = useSelector(selectCurrentUser);

  useEffect(() => {
    const fetchNewsForReporter = async () => {
      try {
        dispatch(loadingSlice.actions.startLoading());
        const startDate = dates?.[0]?.format("YYYY-MM-DD") || "";
        const endDate = dates?.[1]?.format("YYYY-MM-DD") || "";

        const res = await postNewsForReporter(user._id, searchKey, startDate, endDate, selectCategories);
    ...
    fetchNewsForReporter();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchKey, dates, selectCategories]);
  ...
  const items = [
    {
      ...
      children:
        news.length > 0 ? (
          <>
            <NewsItem news={currentRecommends} />
  ...
  return (
    <section className="grid grid-cols-1 lg:grid-cols-1 gap-[10px] mb-[20px]">
      ...
      <div className="flex justify-between items-center w-full gap-[18px] mt-[24px]">
      ...
      <div className="p-[24px] border border-[#E5E5E5] rounded-[8px] mt-[20px] bg-gray-50">
...
export default NewsMangeReporter;
